...
{
    //[ObservableProperty]
    //public List<Dice> _dices = Enumerable.Range(1, 5).Select(index => new Dice(index)).ToList();

    //[RelayCommand]
    //public void HoldDices(int[] position)
    //{
    //    Dices.ForEach(d => d.Release());
    ...
    //{
    //    Dices.Where(dice => !dice.Keep).ToList().ForEach(dice => dice.Roll());
    //}

    //public void ToggleDice(int position)
    //{
    //    Dices.Find(d => d.Position == position)?.Toggle();
    //}
}
